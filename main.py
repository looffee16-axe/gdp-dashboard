...
def get_data():
    ...
    DATA_FILENAME = Path(__file__).parent/'data/Territories.csv'
    dim_territory_df = pd.read_csv(DATA_FILENAME)

    df_list = [fct_df1,fct_df2,fct_df3]

    fct_df = pd.concat(df_list, ignore_index=True)

    df = pd.merge(fct_df,dim_territory_df,left_on='TerritoryKey',right_on='SalesTerritoryKey',how='left')
    ...
    df = df.groupby(['Year','Country']).agg({'OrderQuantity':'sum'}).reset_index()
...
# Filter the data
filtered_df = df[
    (df['Country'].isin(selected_filter))
    & (df['Year'] <= to_year)
    & (from_year <= df['Year'])
]
...
#st.header(f'Sales in {to_year}', divider='gray')
